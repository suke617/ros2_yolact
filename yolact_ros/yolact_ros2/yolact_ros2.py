...
from rcl_interfaces.msg import SetParametersResult  #こいつがあるとパラメータ読んだときに変数として追加できる
from rcl_interfaces.msg import Parameter, ParameterType #parameterを読み込むとparameter.nameでパラメータ名を読み込む
...
class YolactNode(Node):
    def __init__(self, node_name):
        ...
        self.image_pub = self.create_publisher(Image, f'{publish_ns}/visualization',qos_profile=self.qos_profile)
        self.detections_pub = self.create_publisher(Detections, f'{publish_ns}/detections',qos_profile=self.qos_profile)
        
        #パラメータ初期化 initで作製しとかないとエラーがでるので
        self.setParams_()
        #set_parameters_callbackの中のやつがコールバック関数になる
    ...
    #self.get_parameter.valueで値をよびだす。#初期の値は無いのでNoneになると思う
    def setParams_(self):
    ...
    #パラメータ取得　paramsファイルを読み込んだときに実行される
    def parameter_callback_(self, params):
        model_path_changed = False
        for param in params: #パラメータの数だけコールバック
    ...
    def set_subscription_(self):
        if (self.use_compressed_image_):
            self.create_subscription(CompressedImage, '/compressed', self.img_callback_,qos_profile=self.qos_profile)
        else:
            self.create_subscription(Image, self.image_topic_, self.img_callback_,qos_profile=self.qos_profile)
    ...
    def img_callback_(self, msg):
        try:
            if (self.use_compressed_image_):
                ...
                cv_image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR) #メモリキャッシュから画像データを読み取り、画像形式に変換
            else:
                cv_image = self.bridge.imgmsg_to_cv2(msg, "bgr8") #rosの形式からopencvの形式に変更する
    ...
    def evalimage_(self, cv_image, image_header):
        ...
        with torch.no_grad():
            ...
            #fps計測　
            #Durationはhttps://roboticsbackend.com/ros-duration-roscpy-roscpp/ にドキュメントあるよ
